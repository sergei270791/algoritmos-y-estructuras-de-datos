...
public:
	...
	void preOrden(Nodo*);
	void preOrdenPar(Nodo*,Nodo*);
	void preOrdenImpar(Nodo*,Nodo*);
	void par();
	void impar();
	void inOrden(Nodo*);
	void postOrden(Nodo*);
...
{	
	if (r==NULL) //Si no tiene raíz, crea el nodo r
	{
		r=new Nodo;
		r->dato=nuevoDato;
		return;
...
void arbolBB::preOrdenPar(Nodo* raiz,Nodo* raizPar){
	if (raiz != NULL)
	{	
		if(raiz->dato%2==0){
			crear(raizPar,raiz->dato);
		}
		preOrdenPar(raiz->izq,raizPar);
		preOrdenPar(raiz->der,raizPar);
	}
}

void arbolBB::preOrdenImpar(Nodo* raiz,Nodo* raizImpar){
	if (raiz != NULL)
	{	
		if(raiz->dato%2!=0){
			crear(raizImpar,raiz->dato);
		}
		preOrdenImpar(raiz->izq,raizImpar);
		preOrdenImpar(raiz->der,raizImpar);
	}
}
...
void arbolBB::par(){
	arbolBB arbolPar;
	preOrdenPar(raiz,arbolPar.raiz);
	cout<<"\nPor anchura\n";
	arbolPar.porAnchura();
	cout << "\nPreorden\n";
...
/* void arbolBB::impar(){
	arbolBB arbolImpar;
	int* impar=preOrden(raiz,1);
	int n=12;
	arbolImpar.completarArbol(impar,n);
	cout<<"\nPor anchura\n";
