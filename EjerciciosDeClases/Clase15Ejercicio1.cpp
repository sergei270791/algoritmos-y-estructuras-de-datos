...
void crearArcos(GrafoMatriz grafo){
	int i,n,peso;
	n=grafo.getNumDeVertices();
	string arrayElementos[n];
	int arrayOrdenado[n];
	for(i=0;i<n;i++)
	{	
		arrayElementos[i]=grafo.obtenerVertice(i).getNombre();
		arrayOrdenado[i]=stoi(arrayElementos[i]);
	}
	ordBurbuja(arrayOrdenado,n);
	string a,b;
	for (i = 0; i < n-1; i++)
	{	
		a=to_string(arrayOrdenado[i]);
		b=to_string(arrayOrdenado[i+1]);
		peso=arrayOrdenado[i+1]-arrayOrdenado[i];
		grafo.crearNuevoArco(a, b,peso);
	}
}
